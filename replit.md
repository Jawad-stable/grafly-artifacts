...
- **Bottom nav** (`app/(tabs)/_layout.tsx`): slim icon-only floating capsule (height 58, borderRadius 29, blur+border bg). Active tab = filled Phosphor icon in `colors.primary` + 5x5 primary-color dot below. No labels in the bar. No gradients or glow — kept intentionally minimal.
